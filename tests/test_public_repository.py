...
ALLOWED_RESULTS = {
    "results/diagnostic/route_a_prospective_v2/analysis/threshold_headroom.json",
    "results/diagnostic/route_a_prospective_v2/analysis/threshold_headroom.pdf",
...
class PublicRepositoryTests(unittest.TestCase):
    ...
    def test_required_ai_declarations_remain_visible_without_protocol_duplication(self):
        manuscript = (ROOT / "main_neurocomputing.tex").read_text(encoding="utf-8")
        tmlr = (ROOT / "main_tmlr.tex").read_text(encoding="utf-8")
        protocol = (ROOT / "sections" / "03_decision_protocol.tex").read_text(encoding="utf-8")
        disclosure = "ChatGPT was used to suggest specific text and table edits."
        self.assertIn("Declaration of generative AI and AI-assisted technologies", manuscript)
        self.assertEqual(manuscript.count(disclosure), 1)
        self.assertEqual(tmlr.count(disclosure), 1)
        self.assertNotIn("AI-assisted tools supported", protocol)

    def test_no_private_paths_or_informal_process_traces(self):
        tool_name = "".join(("co", "dex"))
        forbidden = (
            "d:\\users\\",
            "documents\\毕业论文",
            f"based on {tool_name}",
            "generated by gemini",
            f"for {tool_name}:",
            f"{tool_name} suggested",
            "3-ai review",
            "fsd-gnn",
            "fsd framework",
            "tkde submission",
