...
| The historical combined rule does not improve over trivial baselines under matched scoring. | The prospective audit at `results/diagnostic/route_a_prospective_v2/analysis/diagnostic_audit.json` reports the complete 11-dataset comparison; the six-architecture restrictions and all 63 subset values are in `equal_budget_sensitivity.json` and `portfolio_robustness.json`. | Supported for the frozen protocol and bounded post-hoc restrictions | “No demonstrated incremental value in this benchmark,” not a universal diagnostic impossibility claim. |
