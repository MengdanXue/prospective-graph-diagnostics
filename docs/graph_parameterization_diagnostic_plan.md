...
Commit the design, config, new execution code and tests before evaluating these datasets. Bind every record and the run manifest to the execution source commit, executable/config fingerprints, environment, pinned NPZ hashes, original split ID, and exact transformed-feature hash and fit metadata. Record all four trial histories, selected trial/checkpoint, train/validation label counts, prediction counts, class recalls, balanced accuracy, loss, majority baseline and dominant prediction fraction. Prepare H2GCN adjacency buffers once per dataset, using the original function.
