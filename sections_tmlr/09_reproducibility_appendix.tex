...
\subsection{Winning Graph Architectures and the Structural Mismatch}

Table~\ref{tab:winning_architectures} reports which graph architecture validation selects on each unit, ordered by training-label homophily. It is a deterministic re-aggregation of the frozen audit produced by \texttt{scripts/summarize\_winning\_architectures.py}; no model is retrained and no action is changed. The heterophily-aware set (GPR-GNN, H2GCN, LINKX) is fixed by the benchmark configuration in Section~\ref{sec:protocol}, not chosen after observing outcomes.

Two facts support Section~\ref{sec:mismatch}. First, heterophily-aware architectures win 95 of 110 units and do so across the whole homophily range, including all ten units on PubMed and all ten on Coauthor-CS. Second, the combined rule grants the graph action on exactly the four most homophilous datasets, while the four datasets carrying 95.1\% of its regret sit below the threshold and receive no graph action at all.

\begin{table}[ht]
\centering
\caption{Validation-selected graph architecture per dataset, ordered by mean training-label homophily $h_1$. ``Het.-aware'' counts units won by GPR-GNN, H2GCN, or LINKX. ``Combined G'' counts graph actions taken by the combined rule after the frozen MLP fallback. Regret is mean full-set oracle-portfolio regret in percentage points.}
\label{tab:winning_architectures}
\scriptsize
\setlength{\tabcolsep}{3.0pt}
\begin{tabular}{@{}lrlrrr@{}}
\toprule
Dataset & $h_1$ & Winning graph architectures & Het.-aware & Combined G & Regret \\
\midrule
