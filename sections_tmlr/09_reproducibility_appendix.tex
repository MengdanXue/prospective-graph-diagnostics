...
\toprule
Dataset & $h_1$ & Selected graph architectures & Het.-aware & Combined G & Regret \\
\midrule
...
Chameleon (2,277 nodes) and Squirrel (5,201 nodes) are the original geometric-preprocessing versions, not the duplicate-filtered variants. Prior work identifies problematic duplicate nodes in these benchmarks~\cite{platonov2023heterophilous}. Our edge canonicalization removes repeated edges, not duplicate nodes, and does not resolve that concern. Section~\ref{sec:subset_robustness} therefore reports their exclusion as a post-hoc sensitivity check; no filtered-graph retraining is claimed.
