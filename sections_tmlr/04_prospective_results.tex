...
The observed homophily values also limit what can be learned about the cutoff. The largest value below $0.50$ is $0.3848$, and the smallest value at or above $0.50$ is $0.7125$; no unit lies in the open interval $(0.3848,0.7125)$. Every threshold strictly inside that interval therefore produces the same 110 actions. The homophily-only versus Combined comparison is an identity after fallback, with difference and interval exactly zero and $p=1$. We retain this comparison in the eight-member Holm family, while recognizing that it contributes no threshold resolution; future data should include graphs with intermediate homophily.
