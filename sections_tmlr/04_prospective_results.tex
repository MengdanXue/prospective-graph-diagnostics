...
\paragraph{Attainable resolution in the observed comparisons.}
An exact two-sided sign-flip test with $k$ nonzero dataset-level differences has a minimum attainable $p$-value of $2^{1-k}$. In the observed always-graph comparison, Cora, CiteSeer, PubMed, and Coauthor-CS contribute zero differences, while the remaining seven differences share one sign. Its raw $p=0.015625$ therefore reaches the floor for this observed zero pattern. It is also the smallest raw $p$-value in the realized eight-comparison family, so Holm's first step gives $8\times0.015625=0.125$ and no hypothesis is rejected. Passing that first step at $\alpha=0.05$ would require at least nine nonzero differences for a sign-flip test attaining its floor. This is a conditional resolution limitation of the observed comparisons, not a claim that the protocol had zero power under every possible outcome: the number of zero differences and the ordering of hypotheses can change with the data. With 11 nonzero differences of one sign, for example, the raw floor would be $2/2^{11}=0.0009765625$. These limitations motivate planning multiplicity and expected dataset heterogeneity before a future study; they do not make non-significance evidence of equivalence.

The operational conclusion is narrower and does not depend on a significance claim in the opposite direction. The combined rule does not demonstrate incremental value: it does not reduce regret relative to always-graph, and its uncertainty does not establish an advantage. Selection accuracy alone would hide this result: the target imbalance makes always-graph strong, while regret shows that the diagnostic's errors are also substantially more costly. Under this protocol the evaluated diagnostics provide no stable incremental decision value. Section~\ref{sec:edge_intervention} next tests whether this failure can be explained by graph structure being uninformative, and Section~\ref{sec:discussion} then discusses a possible diagnostic--portfolio mismatch and its limits.
