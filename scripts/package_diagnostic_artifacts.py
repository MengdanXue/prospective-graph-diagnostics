...
def add_manifest_and_config(
    ...
    config_bytes = git_bytes(repo, manifest["source_commit"], config_path.relative_to(repo).as_posix())
    require(json.loads(config_bytes) == manifest["config"], f"{prefix}: historical config content mismatch")
    add_entry(entries, f"{prefix}/config/{config_path.name}", config_bytes)
